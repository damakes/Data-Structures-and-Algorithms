...
# Exercise 1. Implementing insert method
    '''
    Here you have a class named IntArray with an implementation of an Array that can store integer values.
    It uses another class ReservedMemory (also provided) to store the integer values in memory as bytes (you can convert values to byte type or you can just use integer values between 0 and 255. Both are fine. In the code provided the latter form is used).
    ...
    def insert(self, index:int, val:int) -> None:
        # Validate index to be within bounds
        ...
        new_resmem.copy(self._resmem, count=(self._size - index - 1) * self._bytes_per_element, source_index=index
                        * self._bytes_per_element, destination_index=(index + 1) * self._bytes_per_element)

        # Update the reserved memory to the new one
        self._resmem = new_resmem
        return

# Exercise 2. Implementing remove method
    '''
    Continuing last exercise's example, you have now to implement a remove method for the IntArray class.
    Again, the IntArray and ReservedMemory classes are provided and you only have to add your own code for the remove method.
    The method is also already defined.
    The remove method should allow to remove any element from the array given its index.
    It should raise an IndexError if the index provided is not between the bounds of the array.
    The method returns the value of the removed element or None if the array is empty
    ...
    Tulos
        3 IntArray (5 elements): [0, 1, 2, 4, 5]
    '''

    def remove(self, index:int) -> int:
        ### Remove this comment and the `pass` line and write your code here!
        # Returns the value of None if the array is empty.
        if self._size == 0:
            return None
        ...
        # Shrink the reserved memory if necessary
        if self._size == 0:
            self._resmem = None
        else:
            new_resmem = ReservedMemory(self._size * self._bytes_per_element)
...
# Exercise 3. Implementing a search method
    '''
    This one should be very easy.
    We still have the same code as previous exercises with IntArray.
    Your job is to implement a search method for this class.
    The search method accepts a value to be searched in the array
    and returns the first index position where the value is found or -1 is the value is not found the array
