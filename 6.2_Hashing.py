...
class HashTable():
    ...
    def put(self, key, value):
        ...
        Returns: None
        """
        # return MemoryError if the HastTable is full.
        if self.used_slots == self.size:
            raise(MemoryError('No space in Hash table') )

        h = self._hash(key) # hash value for the given key
        q = self._find_key(h, key) # find key in hash table
        ...
        if q is not None:
            ...
            q = self._find_free_slot(h) # find free slot
            self.slots[q] = HashItem(key, value) # add an element
            self.used_slots +=1 # increase
